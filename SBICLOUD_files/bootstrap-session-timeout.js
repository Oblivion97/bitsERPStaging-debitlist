    ...
    $.sessionTimeout = function (options) {
        ...
        if (!opt.ignoreUserActivity) {
            ...
            $(document).on('keyup mouseup touchend touchmove', function(e) {
                wc.broadcast('resetSession', new Date());
                ...
                if ($('#session-timeout-dialog').length > 0 ) {
                    //Compatibility layer for bootstrap modal 2.x and 3.x
                    var modalInstance = $('#session-timeout-dialog').data('bs.modal') || $('#session-timeout-dialog').data('modal') ;
                    if(modalInstance && modalInstance.isShown) {
                        // http://stackoverflow.com/questions/11519660/twitter-bootstrap-modal-backdrop-doesnt-disappear
                        $('#session-timeout-dialog').modal('hide');
                        $('body').removeClass('modal-open');
                        $('div.modal-backdrop').remove();
                        $('div.modal-scrollable') && $('div.modal-scrollable').hide();
                    }
                }
            });
        }
        ...
        this.startSessionTimer = function startSessionTimer() {
            // Clear session timer
            clearTimeout(timer);
